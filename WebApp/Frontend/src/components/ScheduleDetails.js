...
const ScheduleDetails = ({ schedule }) => {
    ...
    const HandleClick = async () => {
        ...
        if (confirmBox === true) {
            ...
            const res = await fetch('/users/deleteSchedule/' + schedule._id,
                {
                    method: 'DELETE',
                    headers: {
            ...
            if (res.ok) {
                dispatchSchedule({ type: 'DELETE_SCHEDULE', payload: data })
                setTutorialTableView([])
    ...
    return (
        ...
        <div className='teacherAssistantsProfileBody'>
            <div>
                ...
                <span className="tableDelete" onClick={HandleClick} >Delete
