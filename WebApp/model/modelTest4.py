...
def main():
   ...
   # row : TA, col : Course
    ...
    # Each tutorial is assigned to exactly one course in .
    ...
    for course in all_courses:
        for slot in all_slots:
            if (schedule[slot][course] == 1):
                assignmentConstraints = []
                for ta in all_tas:
                    assignmentConstraints.append(tutorials[(ta, slot, course)])
                model.AddBoolOr(assignmentConstraints)
    ...
    for ta in all_tas:
        for course in all_courses:
            if (assignment[ta][course] == 1):
                continue
            for slot in all_slots:
                TACourseMisMatch.append(tutorials[(ta, slot, course)])
    ...
    for slot in all_slots:
        for course in all_courses:
            if (schedule[slot][course] == 1):
                continue
            for ta in all_tas:
                SlotCourseMisMatch.append(tutorials[(ta, slot, course)])
    model.Add(sum(SlotCourseMisMatch) == 0)

    # pylint: disable=g-complex-comprehension
    model.Maximize(sum(tutorials[(ta, slot, course)]
                   for ta in all_tas for slot in all_slots for course in all_courses))
    ...
    if status == cp_model.OPTIMAL or status == cp_model.FEASIBLE:
        ...
        for s in all_slots:
            ...
            for c in all_courses:
                for ta in all_tas:
                    if solver.Value(tutorials[(ta, s, c)]) == 1:
